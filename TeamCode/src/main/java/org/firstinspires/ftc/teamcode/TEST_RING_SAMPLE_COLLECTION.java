...
public class TEST_RING_SAMPLE_COLLECTION extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        //image recognition
        int rings = readStack();
        //we are intentionally leaving the color sensor lights on to prevent any contaminant in the images
        ...
        sleep(30000);
        //just letting the program remain running while we are taking images so that any backlash caused by program ending will not effect sample precision.
...
//            motors[i].setPower(speed);
    ...
    static double f(int x, int n){
        return -Math.pow((2.17 * Math.pow(x - n / 2, 2)) / (n * n), 1.25 /*changed from 1.75 for accuracy*/) + 0.5;
    }
